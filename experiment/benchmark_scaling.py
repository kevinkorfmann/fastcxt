...
Usage:
    CUDA_VISIBLE_DEVICES=2 python benchmark_scaling.py
"""
...
NODE_TIME_WEIGHTS = EXPERIMENT_DIR / "node_time_model.pt"
DEVICE = "cuda:0"
WINDOW_SIZE = 2000
SEQ_LEN = 1_000_000
...
def bench_node_time(model, ts, pairs, tree_feats_t, n_windows, window_size, repeats=3):
    times = []
    for _ in range(repeats):
        torch.cuda.synchronize()
        t0 = time.perf_counter()
        with torch.inference_mode():
            node_times_pred = model(tree_feats_t)
...
def run_benchmark(n_samples: int, base_model, pair_counts: list[int]):
    print(f"\n{'='*80}")
    ...
    for n_pairs in pair_counts:
        ...
        mn, sn = bench_node_time(node_model, ts, pivot, tree_feats_t,
...
def main():
    print("Loading pair-based model ...")
    lit = LitFastCxt.load_from_checkpoint(str(CHECKPOINT_BASE), map_location="cpu")
    ...
    for n_samples in [50, 100, 200]:
        ...
        all_results[n_samples] = run_benchmark(n_samples, base_model, counts)
