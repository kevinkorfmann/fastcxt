...
# ============================================================================
# fastcxt — 1 Mb AnoGam experiment (5000 windows) on Betty cluster
# ============================================================================
# 5000 windows (1 Mb / 200 bp) — 10x context of the original 100kb model
# model: base_trees_5k (d_model=256)
# Saves every checkpoint so we can pick the least overfit one later
# ============================================================================
...
WORK_DIR="/vast/projects/smathi/cohort/fastcxt_1mb_context"
...
SEQ_LEN=1000000       # 1 Mb
WINDOW_SIZE=200       # 200 bp -> 5000 windows
...
if [ "$SKIP_SIM" = false ]; then
    step "Step 1: Simulate AnoGam (1 Mb sequences, 5000 windows) training data"
    T0=$SECONDS
...
# -- Step 3: Train base_trees_5k model with CSV logging + save all ckpts ----
if [ "$SKIP_TRAIN" = false ]; then
    step "Step 3: Train base_trees_5k model (${EPOCHS} epochs, save every checkpoint)"
    T0=$SECONDS
    fastcxt-train \
        --model base_trees_5k \
        ...
        --workers "$NUM_WORKERS" \
        --log-dir "$WORK_DIR/tsinfer_5k_logs" \
        --dataset-mode lazy \
        --csv-log \
        --save-every-epoch
...
echo ""
echo "  Outputs:"
echo "    Logs/Metrics: $WORK_DIR/tsinfer_5k_logs/"
echo "    Simulations:  $SIMS_DIR/"
echo "    Checkpoints:  saved every epoch — pick the least overfit one"
echo ""
