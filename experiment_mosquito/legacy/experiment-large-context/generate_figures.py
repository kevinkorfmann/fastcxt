#!/usr/bin/env python3
"""Generate evaluation figures for the large-context (4000 window) experiment.

Single model: tree-augmented (tsinfer) with 4k context.
Key differences from experiment_mossies:
    SEQ_LEN     = 800_000 bp  (800 kb)
    WINDOW_SIZE = 200 bp
    N_WINDOWS   = 4000

Usage:
    python generate_figures.py \
        --checkpoint tsinfer_4k_logs/lightning_logs/version_0/checkpoints/epoch=29-step=*.ckpt \
        --sims-dir sims \
...
WINDOW_SIZE = 200
SEQ_LEN = 800_000
N_WINDOWS = SEQ_LEN // WINDOW_SIZE  # 4000
MUTATION_RATE = 1e-8
...
def figure_scatter(model, ts, out_dir, pivot_pairs=[(0, 1), (2, 3), (4, 5)]):
    ...
    ax.set_title("AnoGam 4k-context -- Tree-aug (tsinfer)", fontweight="bold",
                 loc="left", fontsize=8, pad=4)

    plt.tight_layout()
    fig.savefig(out_dir / "scatter_anogam_4k.png", bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved scatter_anogam_4k.png  (r={r:.3f}, RMSE={rmse:.3f})")
    return r, rmse
...
def figure_genome(model, ts, out_dir, pivot_pairs=[(0, 1), (2, 3), (4, 5)]):
    ...
    fig.savefig(out_dir / "genome_anogam_4k.png", bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved genome_anogam_4k.png")
...
def figure_residuals(model, ts, out_dir, pivot_pairs=[(0, 1), (2, 3), (4, 5)]):
    ...
    ax_res.set_title("a  AnoGam 4k -- Tree-aug (tsinfer)", fontweight="bold",
                     loc="left", fontsize=8, pad=3)
    ...
    plt.tight_layout()
    fig.savefig(out_dir / "residuals_anogam_4k.png", bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved residuals_anogam_4k.png  (mu={resid.mean():.3f}, sigma={resid.std():.3f})")
...
def figure_training(metrics_path, out_dir):
    ...
    fig.savefig(out_dir / "training_anogam_4k.png", bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved training_anogam_4k.png")
...
def main():
    ap = argparse.ArgumentParser(
        description="Generate large-context (4k) experiment evaluation figures",
    )
    ap.add_argument("--checkpoint", type=str, required=True,
                    help="Path to tsinfer-augmented 4k model checkpoint")
    ...
    if r is not None:
        summary = out_dir / "summary_anogam_4k.csv"
        ...
        print(f"  Saved summary_anogam_4k.csv")
