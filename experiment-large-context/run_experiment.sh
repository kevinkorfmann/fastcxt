...
# ============================================================================
# fastcxt — Large-context AnoGam experiment (8x context, 4000 windows)
# ============================================================================
# Runs the full pipeline: simulate AnoGam via stdpopsim -> preprocess with
# lazy SFS + tsinfer tree features -> train tree-augmented model -> evaluate.
#
# Key differences from experiment_mossies/:
#   - sequence length:    800 kb  (vs 100 kb)
#   - window size:        200 bp  (same)
#   - windows per seq:    4000    (vs 500)
#   - model:              base_trees_4k
#   - preprocessing:      --lazy-sfs (genotype matrix stored, SFS computed on-the-fly)
#   - training:           --dataset-mode lazy
#   - batch size:         16 with grad_accum 16 (effective batch 256)
#
# Usage:
...
SEQ_LEN=800000        # 800 kb
WINDOW_SIZE=200       # 200 bp -> 4000 windows
...
if [ "$SKIP_SIM" = false ]; then
    step "Step 1: Simulate AnoGam (800 kb sequences) training data"
    T0=$SECONDS
...
# -- Step 3: Train tree-augmented (tsinfer) model with 4k context -----------
if [ "$SKIP_TRAIN" = false ]; then
    step "Step 3: Train base_trees_4k model (${EPOCHS} epochs, lazy SFS)"
    T0=$SECONDS
    fastcxt-train \
        --model base_trees_4k \
        ...
        --workers "$NUM_WORKERS" \
        --log-dir "$SCRIPT_DIR/tsinfer_4k_logs" \
        --dataset-mode lazy
...
if [ "$SKIP_FIGURES" = false ]; then
    ...
    CKPT=$(find tsinfer_4k_logs -name "*.ckpt" -type f 2>/dev/null | sort | tail -1 || true)
    if [ -n "$CKPT" ]; then
...
echo ""
echo "  Outputs:"
echo "    Checkpoint:  $SCRIPT_DIR/tsinfer_4k_logs/"
