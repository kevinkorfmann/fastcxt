...
\graphicspath{{experiment/paper_figures/}{experiment_mossies/paper_figures/}{experiment_mossies/data_application/results/}}
...
\begin{enumerate}
  \item \textbf{Mamba-based architecture.} We introduce \textsc{fastcxt}, which replaces the
    ...
    design processes all 500 windows in a single forward pass, capturing long-range
...
positions, we predict the log-TMRCA $y_w = \log T_{ij}(w)$ at each of $W = 500$ non-overlapping
windows of $\Delta = 2{,}000$\,bp spanning a 1\,Mb block. The model outputs per-window mean
...
simulations to test cross-species transfer (Section~\ref{sec:anogam}).

\subsection{Prediction Accuracy}

\begin{table}[t]
\caption{Prediction accuracy on held-out simulations (log-TMRCA, 500 windows, three focal pairs).
Human-like: 1\,Mb, 2\,kb windows; AnoGam: 100\,kb, 200\,bp windows.}
\label{tab:accuracy}
\centering
\begin{tabular}{lcc}
\toprule
\textbf{Model} & $r$ & \textbf{RMSE} \\
\midrule
Base (SFS-only)             & 0.879 & 0.582 \\
Tree-augmented (true trees) & 0.869 & 0.619 \\
Tree-augmented (tsinfer)    & 0.887 & 0.558 \\
Tree-augmented (tsinfer, AnoGam) & 0.804 & 0.646 \\
\bottomrule
\end{tabular}
\end{table}
...
trees carry complementary information beyond the SFS. The AnoGam model (panel~d) attains
$r = 0.804$ on held-out AnoGam simulations, demonstrating cross-species transfer.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig2_combined.png}
...
\emph{Anopheles gambiae} (100\,kb, 200\,bp windows). Dashed line: perfect prediction.}
\label{fig:scatter}
\end{figure}

\subsection{TMRCA Along the Genome}

Figure~\ref{fig:genome} shows predicted TMRCA profiles for one focal pair on 1\,Mb (top) and
100\,kb AnoGam (bottom). Both models capture peaks and valleys in coalescence time, resolve
rapid transitions at recombination breakpoints, and produce well-calibrated $\pm 1$\,s.d.\
uncertainty bands.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig3_combined.png}
\caption{Predicted TMRCA along the genome for a single focal pair. Top: human-like 1\,Mb---red =
true; blue = base; green = tree-aug (true); orange = tree-aug (tsinfer).
Bottom: AnoGam 100\,kb---red = true; orange = tree-aug (tsinfer). Shaded: $\pm 1$\,s.d.}
\label{fig:genome}
\end{figure}
...
Figure~\ref{fig:scatter} and the bottom row of Figure~\ref{fig:genome} show scatter
...
\paragraph{Cross-species generalization.}
The AnoGam experiment (Section~\ref{sec:anogam}) shows that \textsc{fastcxt}'s architecture transfers
...
simulation data and adjusted window size.
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figS1_combined.png}
...
(b)~AnoGam tree-augmented (tsinfer). All models converge smoothly without overfitting.}
\label{fig:training}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figS2_combined.png}
...
human-like 1\,Mb; (d)~AnoGam tree-aug (tsinfer, 100\,kb). Mean and standard deviation
annotated in each panel title. All models show approximately centered, symmetric residuals
with no spatial bias.}
\label{fig:residuals}
\end{figure}
