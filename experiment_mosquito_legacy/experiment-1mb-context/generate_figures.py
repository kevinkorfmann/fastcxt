#!/usr/bin/env python3
"""Generate evaluation figures for the 1 Mb context (5000 window) experiment.

Adapted from experiment-large-context/generate_figures.py.
    SEQ_LEN     = 1_000_000 bp  (1 Mb)
    WINDOW_SIZE = 200 bp
    N_WINDOWS   = 5000

Usage:
    python generate_figures.py \
        --checkpoint tsinfer_5k_logs/.../epoch=50-step=35547.ckpt \
        --sims-dir /vast/projects/smathi/cohort/fastcxt_1mb_context/sims \
...
WINDOW_SIZE = 200
SEQ_LEN = 1_000_000
N_WINDOWS = SEQ_LEN // WINDOW_SIZE  # 5000
MUTATION_RATE = 1e-8
...
def load_model(ckpt_path, device=DEVICE):
    # Checkpoints saved with torch.compile have '_orig_mod.' prefix — strip it
    ...
    torch.save(ckpt, "/tmp/_cleaned_ckpt.pt")
    lit = LitFastCxt.load_from_checkpoint("/tmp/_cleaned_ckpt.pt", map_location="cpu")
...
def find_test_ts(sims_dir):
    ...
    # prefer test split, then n50
    for f in ts_files:
        if "test" in f.parent.name or "test" in f.name:
...
def figure_scatter(model, ts, out_dir, pivot_pairs=[(0, 1), (2, 3), (4, 5)]):
    ...
    ax.set_title("AnoGam 1 Mb (5k windows) -- Tree-aug (tsinfer)",
                 fontweight="bold", loc="left", fontsize=8, pad=4)

    plt.tight_layout()
    fig.savefig(out_dir / "scatter_anogam_1mb.png", bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved scatter_anogam_1mb.png  (r={r:.3f}, RMSE={rmse:.3f})")
    return r, rmse
...
def figure_genome(model, ts, out_dir, pivot_pairs=[(0, 1), (2, 3), (4, 5)]):
    ...
    n_true_windows = true_log.shape[1]
    n_pred_windows = pred_means.shape[1]
    true_pos_kb = (np.arange(n_true_windows) + 0.5) * WINDOW_SIZE / 1e3
    pred_pos_kb = (np.arange(n_pred_windows) + 0.5) * WINDOW_SIZE / 1e3
    ...
    for i, (sa, sb) in enumerate(pivot_pairs):
        ax = axes[i]
        ax.step(true_pos_kb, true_log[i], where="mid", color=RED, lw=0.7,
                alpha=0.85, label="True", zorder=10)

        pred_std = np.sqrt(pred_vars[i])
        ax.fill_between(pred_pos_kb,
                        pred_means[i] - pred_std,
                        pred_means[i] + pred_std,
                        alpha=0.10, color=ORANGE, lw=0)
        ax.plot(pred_pos_kb, pred_means[i], color=ORANGE, lw=0.8, alpha=0.8,
    ...
    fig.savefig(out_dir / "genome_anogam_1mb.png", bbox_inches="tight")
    plt.close(fig)
    print("  Saved genome_anogam_1mb.png")
...
def figure_residuals(model, ts, out_dir, pivot_pairs=[(0, 1), (2, 3), (4, 5)]):
    ...
    ax_res.set_title("a  AnoGam 1 Mb -- Tree-aug (tsinfer)", fontweight="bold",
                     loc="left", fontsize=8, pad=3)
    ...
    plt.tight_layout()
    fig.savefig(out_dir / "residuals_anogam_1mb.png", bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved residuals_anogam_1mb.png  (mu={resid.mean():.3f}, sigma={resid.std():.3f})")
...
def figure_training(metrics_path, out_dir):
    ...
    fig.savefig(out_dir / "training_anogam_1mb.png", bbox_inches="tight")
    plt.close(fig)
    print("  Saved training_anogam_1mb.png")
...
def main():
    ap = argparse.ArgumentParser(
        description="Generate 1 Mb context (5k window) experiment evaluation figures",
    )
    ...
    if r is not None:
        summary = out_dir / "summary_anogam_1mb.csv"
        ...
        print(f"  Saved summary_anogam_1mb.csv")
