...
#SBATCH --output=/vast/projects/smathi/cohort/kkor/fastcxt_repo/experiment_human_poc/slurm_%j.log
set -euo pipefail
...
FROM="${FROM:-01}"
GPUS=0 bash "$REPO_DIR/experiment_human_poc/run_all.sh" --from "$FROM"
