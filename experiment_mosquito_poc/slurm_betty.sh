...
#SBATCH --output=/vast/projects/smathi/cohort/kkor/fastcxt_repo/experiment_mosquito_poc/slurm_%j.log
set -euo pipefail
...
FROM="${FROM:-01}"
GPUS=0 bash "$REPO_DIR/experiment_mosquito_poc/run_all.sh" --from "$FROM"
